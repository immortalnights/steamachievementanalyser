...
module.exports = class Analyzer {
	...
	/**
	 * 10 games with the least achievements remaining
	 * FIXME - not very efficient
	 */
	getHighestCompletionPercentage()
	{
		_.each(this._user.data, function(game, key) {
			if (game.achievements && !game.completed)
			{
				game.appid = key;

				var achieved = _.filter(game.achievements, function(achievement) { return !achievement.achieved; }).length;

				game.percentage = achieved / game.achievements.length;
			}
		});

		var sorted = _.sortBy(this._user.data, 'percentage');
