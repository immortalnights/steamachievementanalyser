...
module.exports = class Analyzer {
	...
	/**
	 * 20 games with the least achievements remaining
	 * FIXME - not very efficient
	 */
	getHighestCompletionPercentage()
	{
		var games = this.getGamesByCompletionPercentage();
		return games.splice(Math.max(games.length - 10, 1)).reverse();
	}

	getLowestCompletionPercentage()
	{
		return this.getGamesByCompletionPercentage().splice(0, 10);
	}
	...
	getEasiestGames()
	{
		let games = this.getGamesByGlobalCompletionPercentage();
		return games.splice(0, 15);
	}
	...
	{
		let easiestAchievements = this.getAchievementsByGlobalCompletionPercentage();
		return _.groupBy(easiestAchievements.splice(0, 25), 'id');
	}

	getGamesByGlobalCompletionPercentage()
	{
		let result = [];
	...
	getAchievementsByGlobalCompletionPercentage(includeNotStarted)
	{
		let result = [];
	...
	getGamesByCompletionPercentage()
	{
		_.each(this._user.data, function(game, key) {
			...
			else if (!game.playtime_forever || !game.achievements)
			{
				percentage = 0;
			}
			else
			{
				...
				let achieved = _.filter(game.achievements, function(achievement) { return !achievement.achieved; }).length;

				percentage = achieved ? (100 - Math.floor((achieved / game.achievements.length) * 100)) : 0;
			}

			game.percentage = percentage;
		});

		// filter out games with 100 or 0 percent
		let results = _.filter(this._user.data, function(game) {
			return game.percentage > 0 && game.percentage < 100;
		});

		return _.sortBy(results, 'percentage');
	}
};
