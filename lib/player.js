	...
	_refreshOwnedGames(key)
	{
		...
		.then(function(ownedGames) {
			...
			_.each(ownedGames, function(game) {
				var userGame = user.data[game.appid];
				...
				if (!userGame)
				{
					...
					user.data[game.appid] = {};

					requiredGames.push(_.pick(game, 'appid'));
				}
				else if (game.playtime_forever !== 0 && game.playtime_forever !== userGame.playtime_forever
		...
		return new Promise(function(resolve, reject) {
			...
			var requests = _.map(requiredGames, function(game) {
				...
				.then(function(gameWithAchievements) {
					...
					if (gameWithAchievements.appid === 221830)
					{
						console.log(gameWithAchievements);
					}

					var userGame = user.data[gameWithAchievements.appid];
